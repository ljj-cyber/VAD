"""
Video-LLaVA Baseline — XD-Violence 评估脚本
...
用法:
  ...
  CUDA_VISIBLE_DEVICES=1 python -m baselines.eval_xd_violence_videollava --max-videos 50
"""
...
# ── 日志 ──
_log_base = "/date/liuzhe/EventVAD/EventVAD/output/baselines/eval_xd_violence_videollava"
...
def load_annotations() -> list[VideoAnnotation]:
    ...
    with open(ANN_FILE) as fp:
        for line in fp:
            ...
            filepath = file_index.get(video_name, "")
    ...
    logger.info(
        ...
        f"{n_anomaly} anomaly + {n_normal} normal, {n_found} files found"
    )
    return annotations
...
def print_metrics(metrics: dict):
    print(f"\n{'='*70}")
    print(f"  Video-LLaVA Baseline — XD-Violence Evaluation Results")
    print(f"{'='*70}")
...
def evaluate(annotations: list[VideoAnnotation], args) -> dict:
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"  Video-LLaVA Baseline — XD-Violence Evaluation")
...
# ── CLI ───────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Video-LLaVA XD-Violence Evaluation")
